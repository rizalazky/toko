...
(function ($) {
  ...
  var checkIsInput = function (target) {
    ...
    return (
      (name === "input" &&
        $.inArray(type, ["text", "password", "file", "search"]) > -1) ||
      name === "textarea"
    );
  };
  ...
  $.Shortcuts.start = function (list) {
    ...
    $(document).bind(
      ($.browser.opera ? "keypress" : "keydown") + ".shortcuts",
      function (e) {
  ...
  $.Shortcuts.add = function (params) {
    if (!params.mask) {
      throw new Error(
        "$.Shortcuts.add: required parameter 'params.mask' is undefined."
      );
    }
    if (!params.handler) {
      throw new Error(
        "$.Shortcuts.add: required parameter 'params.handler' is undefined."
      );
    }

    var type = params.type || "down";
    var listNames = params.list
      ? params.list.replace(/\s+/g, "").split(",")
      : ["default"];
  ...
  $.Shortcuts.remove = function (params) {
    if (!params.mask) {
      throw new Error(
        "$.Shortcuts.remove: required parameter 'params.mask' is undefined."
      );
    }
